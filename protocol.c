...
char *http_thumbnail_cmd = "GET /cgi-bin/camera?resolution=320&quality=1&page=";	//strlen = 50
...
{
	memset (&ptz->adresse, 0, sizeof (struct sockaddr_in));
	ptz->adresse.sin_family = AF_INET;
	ptz->adresse.sin_port = htons (80);
...
g_mutex_lock (&ptz->cmd_mutex);
	...
	if (connect (sock, (struct sockaddr *) &ptz->adresse, sizeof (struct sockaddr_in)) == 0) {
...
g_mutex_lock (&ptz->cmd_mutex);
	...
	memcpy (buffer, http_thumbnail_cmd, 50);
	...
	if (connect (sock, (struct sockaddr *) &ptz->adresse, sizeof (struct sockaddr_in)) == 0) {
		...
		pixbuf_rowstride = gdk_pixbuf_get_rowstride (memory->pixbuf);

		pixels = gdk_pixbuf_get_pixels (memory->pixbuf);

		for (i = 0; i < 180; i++) scanlines[i] = pixels + i * pixbuf_rowstride;
