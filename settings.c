...
void controller_switch_activated (GtkSwitch *controller_switch, GParamSpec *pspec, GtkWidget *controller_ip_adresse_box)
{
	controller_is_used = gtk_switch_get_active (controller_switch);

	gtk_widget_set_sensitive (controller_ip_adresse_box, controller_is_used);

	backup_needed = TRUE;
}

void check_controller_ip_adresse (void)
{
	int i, j, k;
	...
	for (i = 0, k = 0; i < 3; i++)
	{
		...
		memcpy (controller_ip_adresse + k, gtk_entry_buffer_get_text (controller_ip_entry_buffer[i]), j);
		k += j;
		controller_ip_adresse[k] = '.';
		k++;
	}
	j = gtk_entry_buffer_get_bytes (controller_ip_entry_buffer[i]);
	memcpy (controller_ip_adresse + k, gtk_entry_buffer_get_text (controller_ip_entry_buffer[3]), j);
	controller_ip_adresse[k + j] = '\0';

	controller_adresse.sin_addr.s_addr = inet_addr (controller_ip_adresse);

	if (controller_adresse.sin_addr.s_addr == INADDR_NONE) {
		controller_ip_adresse_is_valid = FALSE;
		controller_ip_adresse[0] = '\0';
		for (i = 0; i < 4; i++) gtk_entry_buffer_delete_text (controller_ip_entry_buffer[i], 0, -1);
	} else controller_ip_adresse_is_valid = TRUE;
...
{
	GtkWidget *box1, *frame, *box2, *box3, *box4, *widget, *controller_ip_adresse_box;
	cameras_set_t *cameras_set_itr;
	gint current_page;
	int i, l, k;
	...
	gtk_box_pack_start (GTK_BOX (box1), frame, FALSE, FALSE, 0);
		...
		gtk_container_set_border_width (GTK_CONTAINER (frame), MARGIN_VALUE);
			...
			gtk_widget_set_margin_bottom (box3, MARGIN_VALUE);
				controller_ip_adresse_box = gtk_box_new (GTK_ORIENTATION_HORIZONTAL, 0);
					if (controller_ip_adresse_is_valid) {
				...
				gtk_box_pack_start (GTK_BOX (controller_ip_adresse_box), widget, FALSE, FALSE, 0);
				...
				gtk_box_pack_start (GTK_BOX (controller_ip_adresse_box), widget, FALSE, FALSE, 0);
					...
					g_signal_connect (G_OBJECT (widget), "clicked", G_CALLBACK (check_controller_ip_adresse), NULL);
				gtk_box_pack_start (GTK_BOX (controller_ip_adresse_box), widget, FALSE, FALSE, 0);
				gtk_widget_set_sensitive (controller_ip_adresse_box, controller_is_used);
			gtk_box_pack_end (GTK_BOX (box3), controller_ip_adresse_box, FALSE, FALSE, 0);

				widget = gtk_switch_new ();
				gtk_widget_set_margin_end (widget, MARGIN_VALUE);
				gtk_switch_set_active (GTK_SWITCH (widget), controller_is_used);
				g_signal_connect (widget, "notify::active", G_CALLBACK (controller_switch_activated), controller_ip_adresse_box);
...
void load_config_file (void)
{
	...
	for (i = 0; i < number_of_cameras_sets; i++)
	{
		...
		for (j = 0; j < cameras_set_tmp->number_of_cameras; j++) {
			...
			if (ptz->active) {
				...
				fread (ptz->ip_adresse, sizeof (char), 16, config_file);
				ptz->ip_adresse[15] = '\0';

				ptz->adresse.sin_addr.s_addr = inet_addr (ptz->ip_adresse);

				if (ptz->adresse.sin_addr.s_addr == INADDR_NONE) {
					ptz->ip_adresse_is_valid = FALSE;
					ptz->ip_adresse[0] = '\0';
					cameras_set_with_error = cameras_set_tmp;
				} else ptz->ip_adresse_is_valid = TRUE;
	...
	fread (controller_ip_adresse, sizeof (char), 16, config_file);
	controller_ip_adresse[15] = '\0';
	...
	if (ntohs (tsl_umd_v5_adresse.sin_port) < 1024) tsl_umd_v5_adresse.sin_port = htons (TSL_UMD_V5_UDP_PORT);
...
void save_config_file (void)
{
	...
	for (i = 0; i < number_of_cameras_sets; i++)
	{
		...
		for (j = 0; j < cameras_set_itr->number_of_cameras; j++) {
			...
			if (ptz->active) {
				fwrite (ptz->ip_adresse, sizeof (char), 16, config_file);
	...
	fwrite (controller_ip_adresse, sizeof (char), 16, config_file);
