...
gpointer send_free_d_message (void)
{
	...
	free_d_message[26] = 0x00;
	free_d_message[27] = 0x00;
		...
		if (current_cameras_set != NULL) {
			for (i = 0; i < current_cameras_set->number_of_cameras; i++) {
				...
				if (ptz->is_on) {
					...
					free_d_message[2] = ((unsigned char *)&value)[2];
					free_d_message[3] = ((unsigned char *)&value)[1];
					free_d_message[4] = ((unsigned char *)&value)[0];

					free_d_message[5] = ((unsigned char *)&ptz->free_d_Tilt)[2];
					free_d_message[6] = ((unsigned char *)&ptz->free_d_Tilt)[1];
					free_d_message[7] = ((unsigned char *)&ptz->free_d_Tilt)[0];
					...
					free_d_message[11] = ((unsigned char *)&value)[2];
					free_d_message[12] = ((unsigned char *)&value)[1];
					free_d_message[13] = ((unsigned char *)&value)[0];

					value = ptz->incomming_free_d_Y + ptz->free_d_Y;
					if (value > 8388544) value = 8388544;	//131,071 * 64;
					else if (value < -8388544) value = -8388544;

					free_d_message[14] = ((unsigned char *)&value)[2];
					free_d_message[15] = ((unsigned char *)&value)[1];
					free_d_message[16] = ((unsigned char *)&value)[0];

					value = ptz->incomming_free_d_Z + ptz->free_d_Z + ptz->free_d_optical_axis_height;
					if (value > 8388544) value = 8388544;	//131,071 * 64;
					else if (value < -8388544) value = -8388544;

					free_d_message[17] = ((unsigned char *)&value)[2];
					free_d_message[18] = ((unsigned char *)&value)[1];
					free_d_message[19] = ((unsigned char *)&value)[0];

					free_d_message[20] = ((unsigned char *)&ptz->zoom_position)[2];
					free_d_message[21] = ((unsigned char *)&ptz->zoom_position)[1];
					free_d_message[22] = ((unsigned char *)&ptz->zoom_position)[0];

					free_d_message[23] = ((unsigned char *)&ptz->focus_position)[2];
					free_d_message[24] = ((unsigned char *)&ptz->focus_position)[1];
					free_d_message[25] = ((unsigned char *)&ptz->focus_position)[0];
...
gpointer receive_free_d_message (void)
{
	...
	int msg_len, i;
	unsigned int checksum;
	unsigned char free_d_message[64];
	ptz_t *ptz;
	unsigned char *value;
	...
	while ((msg_len = recvfrom (free_d_input_socket, (char *)free_d_message, sizeof (free_d_message), 0, (struct sockaddr *)&src_addr, &addrlen)) > 1) {
		...
		for (checksum = 0x40, i = 0; i < 28; i++) checksum -= free_d_message[i];

		if ((checksum % 256) != free_d_message[28]) continue;
		...
		if ((current_cameras_set != NULL) && (free_d_message[1] < current_cameras_set->number_of_cameras)) {
			...
			if (ptz->active) {
				...
				value = (unsigned char *)&ptz->incomming_free_d_Pan;

				if (free_d_message[2] & 0x80) value[3] = 0xFF;
				else value[3] = 0x00;
				value[2] = free_d_message[2];
				value[1] = free_d_message[3];
				value[0] = free_d_message[4];

				value = (unsigned char *)&ptz->incomming_free_d_X;

				if (free_d_message[11] & 0x80) value[3] = 0xFF;
				else value[3] = 0x00;
				value[2] = free_d_message[11];
				value[1] = free_d_message[12];
				value[0] = free_d_message[13];

				value = (unsigned char *)&ptz->incomming_free_d_Y;

				if (free_d_message[14] & 0x80) value[3] = 0xFF;
				else value[3] = 0x00;
				value[2] = free_d_message[14];
				value[1] = free_d_message[15];
				value[0] = free_d_message[16];

				value = (unsigned char *)&ptz->incomming_free_d_Z;

				if (free_d_message[17] & 0x80) value[3] = 0xFF;
				else value[3] = 0x00;
				value[2] = free_d_message[17];
				value[1] = free_d_message[18];
				value[0] = free_d_message[19];
...
void start_incomming_free_d (void)
{
	...
	incomming_free_d_thread = g_thread_new (NULL, (GThreadFunc)receive_free_d_message, NULL);
}
